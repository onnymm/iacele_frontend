...
const Sidebar: () => (React.JSX.Element) = (() => {
    ...
    const { isSidebarLocked, isSidebarOpen, setIsSidebarOpen } = useContext(AppContext);
    ...
    return (
        ...
        <div id="sidebar-section" className="top-0 z-20 absolute w-screen h-screen pointer-events-none">

            {/* Elemento que oscurece la vista de la aplicación cuando la barra lateral se muestra */}
            <div className={`${isSidebarOpen && !isSidebarLocked ? "opacity-20 pointer-events-auto": "opacity-0"} absolute transition-opacity bg-black w-screen h-screen`}/>

            <aside ref={sidebarRef} className={`${isSidebarOpen ? 'translate-x-0' : '-translate-x-[288px]'} gap-2 px-4 py-4 lg:px-6 bg-[#162230] flex flex-col absolute w-72 pointer-events-auto transition duration-300 h-screen w-`}>
